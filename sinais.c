...
void ciclo_paralelo(
    ...
    int *branch_taken, int *branch_target, int *stall_pipeline
);
    ...
    while (1) {
        ...
        switch (opcao) {
            ...
            case 2:
                ...
                ciclo_paralelo(&if_id, &id_ex, &ex_mem, &mem_wb, registradores, memory.Dados, &memory, &pc, &pc_prev, &halt_flag, &passos_executados, &branch_taken, &branch_target, &stall_pipeline);
                printf("\n");
...
void ciclo_paralelo(
    ...
    decode(if_id, id_ex, registradores);
     ...
     if (*stall_pipeline) {
        printf("STALL por hazard RAW\n");
        *pc = *pc_prev;  // Mantém o PC antigo
        memset(id_ex, 0, sizeof(ID_EX));  // Insere NOP em ID/EX
        *stall_pipeline = 0;
    } 
